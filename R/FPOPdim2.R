...
#'  
#' @examples
#' data_gen2D(n = 10, chpts = c(2, 4, 6, 8, 10), means1 = c (0, 1, 0, 1, 0), means2 = (1, 2, 3, 4, 5), noise = 1)

data_gen2D <- function(n, chpts, means1, means2, noise = 1){
  #---stop---#
  ...
  if(length(chpts) != length(means1)){stop('changepoints and means1 vectors are of different size')}
  if(length(chpts) != length(means2)){stop('changepoints and means2 vectors are of different size')}
...
#'  
#' @examples
#' plotFPOP2D(data = matrix(c(1,0,1,0,1,0,1,0,2,3,2,3,2,3,2,3), nrow = 2), chpts= c(5, 8), means1 = c(1, 2), means2 = c(0,3))

plotFPOP2D <- function (data, chpts, means1, means2)
{
  #---stop---#
  if (length(data[1,])!= chpts[length(chpts)]){stop('last element of changepoints is always n')}
