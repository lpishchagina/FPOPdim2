...
bool Rect::IsEmpty_rect(){
  if (rectx0 >= rectx1 || recty0 >= recty1) {return true;}
  else { return false;}
}
...
void Rect::Intersection_disk(Disk disk){
  ...
  else{//discriminant/4 if  dxi =< 0 - no intersection points with a line x = xi,if  dxi > 0 - two intersection points with a line x = xi
